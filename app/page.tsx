...
export default function NaveMae() {
  ...
  const [l, setL] = useState([{t:'16:20', x:'Nave Mãe v2.3: Aguardando sincronização do Google Cloud...'}]);
  ...
  const authGmail = () => {
    ...
    window.open(url, '_self'); // Abre na mesma aba para garantir o fluxo
  };
    ...
    <div className='flex h-screen bg-[#09090b] text-white font-sans overflow-hidden'>
      <aside className='w-64 border-r border-zinc-800 p-8 flex flex-col gap-10 bg-black'>
        ...
        <nav className='flex flex-col gap-6 text-zinc-500 text-sm'>
          ...
          <div onClick={authGmail} className='flex items-center gap-4 cursor-pointer hover:text-[#D4AF37] transition-all'><Mail size=20/> GMAIL vkb@kaizen-org.com</div>
        </nav>
      </aside>
      <main className='flex-1 flex flex-col'>
        <header className='h-24 border-b border-zinc-800 flex items-center justify-between px-10 bg-black/40 backdrop-blur-xl'>
          <h1 className='text-xl font-bold uppercase italic tracking-tighter'>Nave Mãe <span className='text-[#D4AF37] font-black'>v2.3</span></h1>
          <Zap className='text-[#D4AF37] animate-pulse' size=24 />
        </header>
        <section className='flex-1 p-10 overflow-y-auto space-y-4 bg-black'>
          {l.map((m, i) => (
            <div key={i} className='p-6 bg-zinc-900/50 border border-zinc-800 rounded-3xl animate-in fade-in'>
              ...
              <p className='text-sm mt-1 font-light leading-relaxed text-zinc-200'>{m.x}</p>
            </div>
          ))}
        </section>
        <form onSubmit={h} className='p-10 border-t border-zinc-800 bg-black/60'>
          <input value={c} onChange={(e)=>setC(e.target.value)} className='w-full bg-zinc-900 border border-zinc-800 rounded-3xl py-7 px-10 outline-none focus:border-[#D4AF37] text-xl font-light' placeholder='Comande a Nave Mãe...' />
        </form>
      </main>
    </div>
  );
